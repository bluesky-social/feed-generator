...
const run = async () => {
  ...
  // YOUR bluesky handle
  // Ex: user.bsky.social
  const handle = 'yochaigal.bsky.social'

  // YOUR bluesky password, or preferably an App Password (found in your client settings)
  // Ex: abcd-1234-efgh-5678

  const password = process.env.APP_PASS as string;

  // A short name for the record that will show in urls
  // Lowercase with no spaces.
  // Ex: whats-hot
  const recordName = 'osr'

  // A display name for your feed
  // Ex: What's Hot
  const displayName = 'OSR Feed'

  // (Optional) A description of your feed
  // Ex: Top trending content from the whole network
  const description = 'An OSR RPG feed'

  // (Optional) The path to an image to be used as your feed's avatar
  // Ex: ~/path/to/avatar.jpeg
  const avatar: string = '/var/www/feed-generator/img/cauldron-black.png'
